...
def allow_groups(update: Update, context: CallbackContext):
    args = context.args
    if not args:
        update.effective_message.reply_text(f"Current state: {KurosakiXrobot.ALLOW_CHATS}")
        return
    if args[0].lower() in ["off", "no"]:
        KurosakiXrobot.ALLOW_CHATS = True
    elif args[0].lower() in ["yes", "on"]:
        KurosakiXrobot.ALLOW_CHATS = False
    else:
